...
export const menuAPI = {
  getAll: () => apiFetch<{ success: boolean; menu: unknown[]; grouped: Record<string, unknown[]> }>("/api/menu"),
...
export const ordersAPI = {
  ...
  getByTable: (tableNumber: number) =>
    apiFetch(`/api/orders?tableNumber=${tableNumber}`, {}, true),
};
...
export const adminAPI = {
  dispatch: (tableNumber: number, waiterId?: string) =>
    apiFetch("/api/admin/dispatch", {
      method: "POST",
      body: JSON.stringify({ tableNumber, waiterId }),
    }, true),

...
export default function OrderPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      router.push(`/confirmation?orderId=${res.order.id}&name=${encodeURIComponent(res.order.studentName)}&table=${tableNum}`);
    } catch (err: any) {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "radial-gradient(ellipse at top, #1e1650 0%, #0d0826 70%)", padding: "0 0 60px" }}>
      ...
      <div style={{ textAlign: "center", padding: "40px 20px 32px" }}>
        ...
        <h1 style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.8rem, 5vw, 2.8rem)", fontWeight: 600, color: "#f5f0e8" }}>
          Place Your Order
        </h1>
      </div>
      ...
      <div style={{ maxWidth: "560px", margin: "0 auto", padding: "0 20px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end" }}>
              <div style={{
                ...
                <p style={{ fontSize: "0.72rem", color: "#9b93b0", marginBottom: "2px" }}>
                  Your table number is on your
                </p>
                <p style={{ fontSize: "0.82rem", color: "#c9a84c", fontWeight: 500 }}>
                  place card or menu card 🪧
                </p>
              </div>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="card" style={{ padding: "32px", textAlign: "center" }}>
            <p style={{ color: "#9b93b0" }}>No menu items available yet. Please check back shortly.</p>
          </div>
        ) : (
          Object.entries(menu).map(([category, items]) => (
            <div key={category} className="card" style={{ padding: "24px", marginBottom: "20px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
                ...
                <span style={{ fontSize: "0.7rem", color: "#9b93b0", background: "rgba(255,255,255,0.05)", padding: "2px 8px", borderRadius: "10px" }}>
                  Pick one
                </span>
              </div>
                ...
                {(items as MenuItem[]).map((item: MenuItem) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        <p style={{
                          fontWeight: 500, color: isUnavailable ? "#9b93b0" : "#f5f0e8",
                        ...
                        {item.description && (
                          <p style={{ fontSize: "0.8rem", color: "#9b93b0" }}>{item.description}</p>
                        )}
                      </div>

...
export default function OrderPage() {
  ...
  const [selected, setSelected] = useState<Record<string, string>>({}); // category → menuItemId
  const [selectedVariants, setSelectedVariants] = useState<Record<string, string>>({}); // menuItemId → variant
    ...
    const fetchData = async () => {
      try {
        ...
        if (course) {
          ...
          if (alreadyOrdered) setHasOrdered(true);
  ...
  const handleSelect = (category: string, itemId: string) => {
    ...
    // Clear variant selection when item changes
  ...
  const handleSubmit = async () => {
    if (!studentName.trim()) { toast.error("Please enter your full name."); return; }
    ...
    // Validate that all selected items with variants have a variant chosen
    ...
    for (const itemId of selectedItems) {
      ...
      if (menuItem && menuItem.variants && menuItem.variants.length > 0) {
        if (!selectedVariants[itemId]) {
          toast.error(`Please choose an option for "${menuItem.name}".`);
          return;
        }
      }
    }
    ...
    setSubmitting(true);
    try {
      // Build items with variant info embedded in the name
  ...
  return (
    <main style={{ minHeight: "100vh", background: "radial-gradient(ellipse at top, #1e1650 0%, #0d0826 70%)", padding: "0 0 60px" }}>
      ...
      <div style={{ textAlign: "center", padding: "40px 20px 32px" }}>
        ...
        {courseInfo && (
          <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "rgba(201,168,76,0.12)", border: "1px solid rgba(201,168,76,0.3)", borderRadius: "20px", padding: "8px 20px" }}>
            <span style={{ fontSize: "18px" }}>{courseInfo.emoji}</span>
            <span style={{ color: "#e8c97e", fontWeight: 600, fontSize: "0.9rem" }}>{courseInfo.label} is now open</span>
          </div>
        )}
      </div>
      ...
      <div style={{ maxWidth: "560px", margin: "0 auto", padding: "0 20px" }}>
        ...
        {!loading && !activeCourse && (
          <div className="card" style={{ padding: "48px", textAlign: "center", marginBottom: "20px" }}>
            ...
            <p style={{ color: "#9b93b0", lineHeight: 1.7 }}>Ordering is not open yet. Please wait for the announcement.</p>
            <p style={{ color: "#c9a84c", fontSize: "0.85rem", marginTop: "16px", fontStyle: "italic" }}>Enjoy the evening — we'll let you know when it's time!</p>
          </div>
        )}
        ...
        {!loading && activeCourse && hasOrdered && (
          <div className="card" style={{ padding: "48px", textAlign: "center", marginBottom: "20px" }}>
            <p style={{ fontSize: "3rem", marginBottom: "16px" }}>✅</p>
            <h2 style={{ fontFamily: "var(--font-display)", fontSize: "1.6rem", color: "#f5f0e8", marginBottom: "12px" }}>You're all set!</h2>
            <p style={{ color: "#9b93b0", lineHeight: 1.7 }}>
              You've already placed your <strong style={{ color: "#c9a84c" }}>{courseInfo?.label}</strong> order.
            </p>
            <p style={{ color: "#9b93b0", lineHeight: 1.7, marginTop: "8px" }}>
              {activeCourse !== "DESSERT" ? `Come back when it's time to order your ${courseInfo?.next}!` : "That's all for tonight. Enjoy your dessert! 🍰"}
            </p>
            <p style={{ color: "#c9a84c", fontSize: "0.85rem", marginTop: "16px", fontStyle: "italic" }}>Enjoy your meal! {courseInfo?.emoji}</p>
          </div>
        )}
                    ...
                    {(items as MenuItem[]).map((item) => {
                      ...
                      return (
                        <div key={item.id}>
                          ...
                          >
                            <div style={{ flex: 1 }}>
                              ...
                              {hasVariants && !isSelected && (
                                <p style={{ fontSize: "0.75rem", color: "#c9a84c", marginTop: "2px" }}>
                                  Choose: {item.variants!.join(" / ")}
                                </p>
                              )}
                            </div>
                          ...
                          {/* Variant dropdown — shows only when item is selected and has variants */}
                          {isSelected && hasVariants && (
                            <div style={{
                              background: "rgba(201,168,76,0.06)", border: "1.5px solid rgba(201,168,76,0.6)",
                              borderTop: "1px solid rgba(201,168,76,0.2)", borderRadius: "0 0 10px 10px",
                              padding: "14px 16px",
                            }}>
                                  ...
                                  return (
                                    <button
                                      ...
                                      style={{
                                        ...
                                        background: isChosen ? "rgba(201,168,76,0.2)" : "rgba(255,255,255,0.04)",
                                        outline: isChosen ? "1.5px solid rgba(201,168,76,0.5)" : "1.5px solid transparent",
                                        transition: "all 0.15s ease", textAlign: "left", width: "100%",
                                      }}
                                    >
                                      <div style={{
                                        width: "16px", height: "16px", borderRadius: "50%", flexShrink: 0,
                                        border: isChosen ? "none" : "2px solid rgba(201,168,76,0.4)",
                                        background: isChosen ? "#c9a84c" : "transparent",
                                        display: "flex", alignItems: "center", justifyContent: "center",
                                      }}>
                                        {isChosen && <span style={{ fontSize: "9px", color: "#0d0826", fontWeight: 700 }}>✓</span>}
                                      </div>
                                      <span style={{ color: isChosen ? "#f5f0e8" : "#9b93b0", fontWeight: isChosen ? 500 : 400, fontSize: "0.9rem" }}>
                                        {variant}
                                      </span>
                                    </button>
                                  );

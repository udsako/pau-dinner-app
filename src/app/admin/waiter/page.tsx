  ...
  const fetchDispatched = useCallback(async () => {
    try {
      ...
      for (const table of allTables) {
        // For each table, get all dispatched orders grouped by course
        try {
          const ordersRes: any = await ordersAPI.getByTable(table.tableNumber);
          const orders = ordersRes.orders || [];

          // Group dispatched orders by course
          const courses: Course[] = ["STARTER", "MAIN", "DESSERT"];
          for (const course of courses) {
            const courseDispatched = orders.filter(
              (o: any) => o.course === course && o.status === "DISPATCHED"
            );
            ...
            allTickets.push({
              ...
              course,
              summary,
              orderCount: courseDispatched.length,
              dispatchedAt: table.dispatchedAt || "",
            });
          }
        } catch { /* skip this table */ }
      }

      // Sort by course order then table number
    ...
    <div style={{ padding: "40px", maxWidth: "1000px" }}>
      ...
      <div style={{ marginBottom: "32px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
          <h1 style={{ fontFamily: "var(--font-display)", fontSize: "2.2rem", color: "#f5f0e8" }}>Dispatched Orders</h1>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <p style={{ fontSize: "0.78rem", color: "#9b93b0" }}>
              Last updated {lastUpdated.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
            </p>
            <button onClick={fetchDispatched} style={{ background: "rgba(201,168,76,0.1)", border: "1px solid rgba(201,168,76,0.3)", borderRadius: "8px", padding: "6px 14px", cursor: "pointer", color: "#c9a84c", fontSize: "0.8rem", fontFamily: "var(--font-body)" }}>
              ↻ Refresh
            </button>
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "12px", marginBottom: "28px" }}>
        {[
          ...
          <div key={stat.label} className="card" style={{ padding: "14px 18px" }}>
            <p style={{ fontSize: "0.65rem", color: "#9b93b0", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "6px" }}>{stat.label}</p>
            <p style={{ fontFamily: "var(--font-display)", fontSize: "1.8rem", color: stat.color }}>{loading ? "—" : stat.value}</p>
          </div>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="card" style={{ padding: "64px", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "var(--font-display)", fontSize: "1.6rem", color: "#f5f0e8", marginBottom: "8px" }}>No dispatched orders yet</h2>
          <p style={{ color: "#9b93b0" }}>Orders will appear here once the admin dispatches a table.</p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          {filtered.map((ticket, i) => {
            ...
            return (
              <div key={`${ticket.tableNumber}-${ticket.course}-${i}`} className="card" style={{ padding: "22px", borderLeft: `4px solid ${config.color}` }}>
                {/* Ticket header */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px", flexWrap: "wrap", gap: "10px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                    <div style={{ width: "52px", height: "52px", borderRadius: "10px", background: config.bg, border: `1px solid ${config.border}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <span style={{ fontFamily: "var(--font-display)", fontSize: "1.5rem", color: config.color }}>{ticket.tableNumber}</span>
                    </div>
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
                        <h2 style={{ fontFamily: "var(--font-display)", fontSize: "1.2rem", color: "#f5f0e8" }}>Table {ticket.tableNumber}</h2>
                        <span style={{ fontSize: "0.72rem", padding: "2px 10px", borderRadius: "20px", background: config.bg, border: `1px solid ${config.border}`, color: config.color, fontWeight: 700 }}>
                ...
                {/* Food summary */}
                <div>
                  <p style={{ fontSize: "0.68rem", color: "#9b93b0", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "10px" }}>Bring to table</p>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                    {Object.entries(ticket.summary).map(([item, count]) => (
                      <div key={item} style={{ display: "flex", alignItems: "center", gap: "8px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(201,168,76,0.15)", borderRadius: "8px", padding: "8px 14px" }}>
                        <span style={{ fontFamily: "var(--font-display)", fontSize: "1.2rem", color: config.color }}>×{count}</span>
                        <span style={{ color: "#f5f0e8", fontSize: "0.9rem", fontWeight: 500 }}>{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
